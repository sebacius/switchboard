...
services/
├── signaling/
│   ├── app/app.go            # Main coordinator
│   ├── b2bua/
│   │   ├── service.go        # CallService interface
│   │   ├── call_service.go   # Implementation
│   │   ├── originator.go     # Outbound INVITE handling
│   │   ├── leg.go            # Leg interface and implementation
│   │   ├── bridge.go         # Bridge interface
│   │   └── bridge_impl.go    # Bridge implementation
│   ├── dialog/
│   │   ├── dialog.go         # Dialog struct
│   │   └── manager.go        # Dialog lifecycle
│   ├── dialplan/
│   │   ├── executor.go       # Route matching
│   │   ├── actions.go        # Action implementations
│   │   └── session_impl.go   # CallSession implementation
│   ├── location/
│   │   ├── binding.go        # AOR binding
│   │   └── store.go          # In-memory store
│   └── routing/
│       └── invite.go         # InviteHandler
├── rtpmanager/
│   ├── server/server.go      # gRPC implementation
│   ├── session/manager.go    # Session lifecycle
│   ├── media/service.go      # Audio playback
│   ├── bridge/manager.go     # RTP bridging
│   └── sdp/builder.go        # SDP generation
└── ui/
